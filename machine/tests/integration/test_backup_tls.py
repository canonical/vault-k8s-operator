...
async def minio_endpoint_and_ca_cert(
    ops_test: OpsTest, host_ip: str
...
async def test_given_self_signed_tls_endpoint_and_ca_chain_when_create_backup_then_succeeds_with_prefixed_key(
    ops_test: OpsTest,
    deploy: VaultInit,
    minio_endpoint_and_ca_cert: tuple[str, str],
):
    endpoint, ca_cert = minio_endpoint_and_ca_cert
    backup_id = await configure_s3_and_create_backup(
        ops_test,
...
@pytest.mark.abort_on_fail
async def test_given_path_set_when_list_backups_then_keys_are_prefixed(
    ops_test: OpsTest,
    deploy: VaultInit,
):
    backup_ids = await list_backups(ops_test, skip_verify=False)
    assert backup_ids, "Expected at least one backup"
    assert all(bid.startswith(f"{S3_PATH}/") for bid in backup_ids), backup_ids


@pytest.mark.abort_on_fail
async def test_given_prefixed_backup_when_restore_backup_then_succeeds(
    ops_test: OpsTest,
    deploy: VaultInit,
):
    restored_id = await restore_backup(
        ops_test,
        root_token=deploy.root_token,
        kv_secret_value="tls-value",
        skip_verify=False,
...
@pytest.mark.abort_on_fail
async def test_given_legacy_root_level_backup_when_restore_backup_then_falls_back(
    ops_test: OpsTest,
    deploy: VaultInit,
    minio_endpoint_and_ca_cert: tuple[str, str],
):
