...
class CertificateRequestErrorCode(str, Enum):
    """Canonical error codes for failed certificate requests."""
...
class _CertificateError(pydantic.BaseModel):
    """Normalized provider-agnostic error reported for a CSR."""

    code: CertificateRequestErrorCode
    message: str
    category: str = "policy"
...
class _ProviderApplicationData(_DatabagModel):
    ...
    # New: errors reported per CSR (additive, backward compatible)
...
class CertificateDeniedEvent(EventBase):
    ...
    def restore(self, snapshot: dict):
        ...
        if IS_PYDANTIC_V1:
            self.error = _CertificateError.parse_raw(snapshot["error"])  # type: ignore[attr-defined]
        else:
            self.error = _CertificateError.model_validate_json(snapshot["error"])
...
class TLSCertificatesRequiresV4(Object):
    ...
    def _load_provider_request_errors(self) -> List[_RequestError]:
        """Load provider request_errors entries (failure reports)."""
        relation = self.model.get_relation(self.relationship_name)
        if not relation:
            logger.debug("No relation: %s", self.relationship_name)
            return []
        ...
        try:
            ...
            logger.warning("Invalid relation data")
            return []
        return provider_relation_data.request_errors
    ...
    def _find_available_certificates(self):
        ...
        # Emit failure events for provider request_errors that match our CSRs
        for entry in self._load_provider_request_errors():
            try:
            ...
            if str(csr_obj) not in [str(csr) for csr in csrs]:
                continue
...
class TLSCertificatesProvidesV4(Object):
    ...
    def _dump_provider_certificates(self, relation: Relation, certificates: List[_Certificate]):
        try:
            # Preserve existing request_errors when updating certificates
            try:
                provider_data = _ProviderApplicationData.load(relation.data[self.model.app])
            except DataValidationError:
                provider_data = _ProviderApplicationData(certificates=[], request_errors=[])
    ...
    def set_relation_error(
        ...
        if not certificates_relation:
            raise TLSCertificatesError(f"Relation {self.relationship_name} does not exist")
        # Ensure exclusivity: remove any certificate entry for the same CSR
        self._remove_provider_certificate(
            relation=certificates_relation,
        ...
        try:
            provider_data = _ProviderApplicationData.load(certificates_relation.data[self.charm.app])
        except DataValidationError:
            provider_data = _ProviderApplicationData(certificates=[], request_errors=[])
        # Remove prior error for this CSR, if any
        new_errors: List[_RequestError] = []
        for entry in provider_data.request_errors:
            try:
                if (
                ...
                # If unparsable, keep it to avoid data loss
                pass
        ...
        provider_data.request_errors = new_errors
        # Dump updated provider data (preserving certificates)
        try:
            provider_data.dump(certificates_relation.data[self.model.app])
        except ModelError:
            logger.warning("Failed to update relation data with request error")
        self._security_logger.log_event(
            ...
            code=provider_error.error.code.value,
        )
    ...
    def _remove_request_error_for_csr(
        ...
        for entry in provider_data.request_errors:
            try:
                ...
                # Keep unparsable entries to avoid data loss
                pass
            filtered.append(entry)
        if len(filtered) != len(provider_data.request_errors):
            provider_data.request_errors = filtered
            try:
                provider_data.dump(relation.data[self.model.app])
            except ModelError:
                logger.warning("Failed to update relation data when removing request error")
