...
class PKIManager:
    """This class manages the PKI backend in Vault."""
    ...
    def __init__(
        ...
        *,
        ...
        self._vault_pki = vault_pki  # TODO: Can we just construct this in the class?
    ...
    def _get_pki_intermediate_ca(self) -> tuple[ProviderCertificate | None, PrivateKey | None]:
    ...
    def _get_current_vault_ca_certificate(self) -> Certificate | None:
        try:
            intermediate_ca_cert = self._client.get_intermediate_ca(mount=self._mount_point)
    ...
    def _intermediate_ca_exceeds_role_ttl(self, intermediate_ca_certificate: Certificate) -> bool:
        ...
        """
        current_ttl = self._client.get_role_max_ttl(role=self._role_name, mount=self._mount_point)
        if (
            not current_ttl
    ...
    def configure(self):
        ...
        if not self._juju_facade.relation_exists(TLS_CERTIFICATES_PKI_RELATION_NAME):
            ...
            return
        self._client.enable_secrets_engine(SecretsBackend.PKI, self._mount_point)
        ...
        if not provider_certificate or not private_key:
            return
        existing_ca_certificate = self._get_current_vault_ca_certificate()
        if existing_ca_certificate and existing_ca_certificate == provider_certificate.certificate:
            if not self._intermediate_ca_exceeds_role_ttl(existing_ca_certificate):
        ...
        self._client.import_ca_certificate_and_key(
        ...
        if not self._client.is_common_name_allowed_in_pki_role(
            role=self._role_name,
            mount=self._mount_point,
            common_name=self._common_name,
        ) or issued_certificates_validity != self._client.get_role_max_ttl(
            role=self._role_name, mount=self._mount_point
        ):
            self._client.create_or_update_pki_charm_role(
        ...
        try:
            self._client.make_latest_pki_issuer_default(mount=self._mount_point)
    ...
    def _generate_pki_certificate_for_requirer(self, requirer_csr: RequirerCertificateRequest):
        ...
        if not self._client.is_pki_role_created(role=self._role_name, mount=self._mount_point):
        ...
        certificate = self._client.sign_pki_certificate_signing_request(
            mount=self._mount_point,
            role=self._role_name,
