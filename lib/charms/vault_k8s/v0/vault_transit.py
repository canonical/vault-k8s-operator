...
from ops import model  # type: ignore
...
class VaultTransitProvides(ops.Object):
    ...
    on: VaultTransitProvidesEvents = VaultTransitProvidesEvents()
    ...
    def set_vault_url(self, relation: ops.Relation, address: str) -> None:
        ...
        relation.data[relation.app].update({"address": address})

    def set_token_secret_id(self, relation: ops.Relation, token_secret_id: str) -> None:
        """Set the token secret id in the relation."""
        # TODO: Is this even necessary? Can we just use the label to look it up?
        if relation.app is None:
            raise ValueError("The `relation.app` is not set")
        relation.data[relation.app].update({"token_secret_id": token_secret_id})
...
class VaultTransitRequires(ops.Object):
    ...
    on: VaultTransitRequireEvents = VaultTransitRequireEvents()
    ...
    @staticmethod
    def get_token_secret_id(relation: ops.Relation) -> Optional[str]:
        """Return the token secret id from the relation."""
        if relation.app is None:
            return None
        return relation.data[relation.app].get("token_secret_id")

    def get_token(self, token_secret_id: str) -> Optional[str]:
        """Return the token from the Juju secret."""
        # TODO: Should we use a label instead?
        secret_content = self.model.get_secret(id=token_secret_id).get_content()
        return secret_content.get("token")

    def get_token_from_relation(self, relation: ops.Relation) -> Optional[str]:
        """Return the token from the relation."""
        token_secret_id = VaultTransitRequires.get_token_secret_id(relation)
        return self.get_token(token_secret_id) if token_secret_id else None
