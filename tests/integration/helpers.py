...
def get_vault_pki_intermediate_ca_common_name(root_token: str, endpoint: str, mount: str) -> str:
    ...
    ca_cert = read_ca_certificate(client, endpoint, mount)
    loaded_certificate = x509.load_pem_x509_certificate(ca_cert)
    return str(
...
def read_ca_certificate(client: hvac.Client, endpoint: str, mount: str) -> bytes:
    for _ in range(3):
        try:
            cert = client.secrets.pki.read_ca_certificate(mount_point=mount)
            return cert.encode("utf-8")
        except hvac.exceptions.InvalidPath:
            time.sleep(30)
    raise Exception("Failed to read the CA certificate")
